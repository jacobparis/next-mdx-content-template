...
The `Navigable` component provides smart prefetching for better performance:

```tsx
<Navigable.Root href={`/post/${post.slug}`} prefetch="hover">
  <Navigable.Link>
    <h2>{post.title}</h2>
  </Navigable.Link>
</Navigable.Root>
...
**How it works**:
- `prefetch="hover"` → Prefetch on mouse enter (default)
- `prefetch="viewport"` → Prefetch when visible in viewport
- Only prefetches when the condition is met, saving bandwidth
- Makes navigation feel instant by loading pages before click

This is especially useful for blog post lists where users hover over links before clicking.
